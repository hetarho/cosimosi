-- +goose Up
-- Signed, accumulated neighbor forgetting nudge in universe-days ([R5]). Written on
-- RECALL to a memory's NEIGHBORS (never to the recalled memory itself — it recovers wholly [F5]);
-- forgetting reads it as effectiveElapsed = max(0, (now − last_recalled_universe_time) + offset). Negative
