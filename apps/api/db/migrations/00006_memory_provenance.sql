...
-- One row per representational event. NEVER updated while retained (append-only [I1]); only the
-- post-window user full-delete sweep deletes it, via the parent memory's ON DELETE CASCADE.
...
-- synthesized at READ from the memory's creation facts, never backfilled — so a memory with zero
-- reconsolidations still yields a one-entry 변천사. Reconsolidation appends kind='reconsolidated'/
-- source='user'; semanticization appends kind='semanticized'/source='system'.
...
-- The 변천사 panel reads one user's memory events in
