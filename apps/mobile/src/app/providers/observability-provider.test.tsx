...
function DiagnosticsFlagProbe() {
  ...
  return <Text>{observability.getFeatureFlag('platform.diagnosticsSurface') ? 'enabled' : 'disabled'}</Text>;
}
...
describe('MobileObservabilityProvider', () => {
  ...
  it('closes the previous Sentry client before reinitializing vendor config', async () => {
    ...
    await waitFor(() => expect(Sentry.init).toHaveBeenCalledWith(expect.objectContaining({dsn: 'dsn-1'})));

    rerender(<MobileObservabilityProvider sentryDsn="dsn-2" release="2" />);

    await waitFor(() => expect(Sentry.close).toHaveBeenCalledTimes(1));
    expect(Sentry.init).toHaveBeenLastCalledWith(expect.objectContaining({dsn: 'dsn-2', release: '2'}));
  });
});
