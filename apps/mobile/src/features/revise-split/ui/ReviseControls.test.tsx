...
const memories: EditableMemoryView[] = [{id: 'a', name: 'Morning', mood: 'JOY', neurons: [{name: 'cafe'}]}];
...
describe('ReviseControls editable surface (mobile)', () => {
  ...
  it('exposes only name / emotion selection / neuron membership — no strength/position/time control', () => {
    const view = render(
      <ReviseControls memories={memories} onRename={noop} onSetMood={noop} onMerge={noop} onSplit={noop} onRevise={noop} />,
    );
    ...
    const moodChips = view.getAllByRole('button').filter(node => node.props.accessibilityState?.selected !== undefined);
