...
describe('buildUniverseGraph', () => {
  it('projects the domain mirror onto the sim contract, dropping unknown references', () => {
  ...
  it('feeds layout from connectivity only, with no emotion term or coordinate [I3][I5]', () => {
    const graph = buildUniverseGraph(universeFixture());

    const inputs = [...graph.neurons, ...graph.synapses, ...graph.episodicMemories, ...graph.activations];
