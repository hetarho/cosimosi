...
function createWorkerSimBridge(spawner: SimWorkerSpawner): UniverseSimBridge {
  ...
  return {
    ...
    start(graph) {
      ...
      stop(false);
      const floats =
        (graph.neurons.length + graph.episodicMemories.length) * FORCE_SIM_COORDINATE_STRIDE;
      const spawned = spawner();
      spawned.onmessage = event => {
        const message = event.data as SimWorkerResponse | null;
        if (spawned !== worker || !message) {
          return;
        }
        ...
        if (previous) {
          spareBuffers.push(previous.buffer as ArrayBuffer);
        }
        inFlight = false;
      };
      spawned.onerror = () => {
