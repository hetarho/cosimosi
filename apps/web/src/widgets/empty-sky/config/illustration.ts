...
/**
 * The empty sky's authored ramp — invented moods in invented proportions, never anyone's data.
 *
 ...
 * 122 — RELIEF's spring green, CALM's teal) are the accent. The public surfaces' sky is the
...
/**
 * How much faster than the product's own sky this one runs — the sky's seconds uniform, multiplied.
 *
 * The product's pace is tuned for a place you live in, where the sky moving is something you notice
 * over a session. A public surface has one: a visitor is here for a few seconds, and at 1× the wash
 * is a still image to them. This is the ONLY thing the public surfaces accelerate, and it
 * accelerates a drift, not a mechanic — nothing about a memory, a strength or a decay is read from
 * this number.
 */
export const EMPTY_SKY_RATE = 2.5
