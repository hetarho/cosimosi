...
export function WritingFlowSheet() {
  ...
  // to decide the past-date notice. A loading/errored read leaves it null → treated as not-past.
  ...
  const runLaunch = useCallback(() => {
    ...
    requestLaunchStars(transport, { body, diaryDate, memories })
      .then((response) => {
        // Insert one optimistic star per id the server created — a past-dated launch saves the
        // diary but returns no ids, so no star appears ([T1][I10]).
        insertLaunchedMemories(memories, response.memoryIds, diaryDate)
        announce(response.newNeuronIds)
        void queryClient.invalidateQueries({ queryKey: createGetUniverseQueryKey(transport) })
        send({ type: 'LAUNCH_OK' })
      })
  ...
  return (
    <>
      <Button variant="primary" className="pointer-events-auto" onClick={open}>
        ...
        <div className="flex flex-col gap-4">
          ...
          {status === 'writing' ? (
            <>
              ...
              <Button variant="primary" className="self-start" disabled={body.trim().length === 0} onClick={runSplit}>
          ...
          {status === 'reviewing' ? (
            <>
              ...
              <div className="flex items-center justify-between gap-3">
                <Button variant="secondary" disabled={busy} onClick={() => send({ type: 'BACK' })}>
