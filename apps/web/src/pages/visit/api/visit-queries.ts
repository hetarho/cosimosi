...
import {
  ...
  VisitService,
  transport,
  type GetResonanceBridgesResponse,
...
import { toSynapseEdge, useSynapseStore, type SynapseEdge } from '@/entities/synapse'
...
/** 공개 스냅샷 → 우주 도메인(별·시냅스). PURE: 스토어를 건드리지 않는다(헌법4 매핑 분리) — 방문
 *  단일 뷰는 `applySharedUniverse`가 이 결과를 스토어에 싣고, 겹쳐보기(spec 37)는 같은 결과를 친구
 *  우주 PROPS로 OverlayUniverse에 넘긴다(두 우주를 한 씬에 — 싱글턴 스토어로는 불가). SharedStar엔
 *  id가 없으므로 응답 인덱스로 합성 id(`shared-N`)를 부여하고(겹쳐보기 다리도 같은 규약을 공유),
 *  시냅스는 인덱스 쌍을 같은 합성 id로 잇는다. */
export function mapSharedUniverse(res: GetSharedUniverseResponse): {
  stars: StarNode[]
  edges: SynapseEdge[]
} {
...
// 겹쳐보기(spec 37) 공명 다리 — 인증 ShareService.GetResonanceBridges. 인증 transport를 쓴다(호출자
// 신원이 있어야 공명 매칭). 다리 정보는 라이브 공명·공유 상태에 묶이므로(소유자가 공유를 끄면 NotFound)
// 방문 쿼리처럼 캐시하지 않는다 — 매 진입이 서버에 재검증한다. enabled는 호출부가 정한다(로그인+겹침 on).
const shareClient = createClient(ShareService, transport)

export function resonanceBridgesQueryOptions(slug: string, enabled: boolean) {
  return queryOptions({
    queryKey: ['resonance-bridges', slug],
    queryFn: ({ signal }: { signal: AbortSignal }): Promise<GetResonanceBridgesResponse> =>
      shareClient.getResonanceBridges({ slug }, { signal }),
    enabled,
    staleTime: 0,
    gcTime: 0,
    refetchOnMount: 'always',
