...
export function isDemoPersona(value: unknown): value is DemoPersona {
  return typeof value === 'string' && PERSONA_ID_SET.has(value)
}

export function parseDemoPersona(value: unknown, fallback: DemoPersona = DEFAULT_PERSONA): DemoPersona {
  return isDemoPersona(value) ? value : fallback
}
...
const FLOW_VALUES = ['not_started', 'persona_selected', 'free', 'tutorial_tbd', 'tutorial'] as const
export type DemoFlow = (typeof FLOW_VALUES)[number]
const FLOW_VALUE_SET = new Set<string>(FLOW_VALUES)
const DEFAULT_FLOW: DemoFlow = 'not_started'

export function isDemoFlow(value: unknown): value is DemoFlow {
  return typeof value === 'string' && FLOW_VALUE_SET.has(value)
}

export function parseDemoFlow(value: unknown, fallback: DemoFlow = DEFAULT_FLOW): DemoFlow {
  return isDemoFlow(value) ? value : fallback
}
