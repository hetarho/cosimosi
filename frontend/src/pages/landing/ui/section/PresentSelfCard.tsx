 ...
 *
 * 토글은 보여주기 위한 손잡이일 뿐이다(spec 22). 실제 우주에서 "요즘의 마음"은 직접 고르는
 * 값이 아니라, 최근 ~6시간 안에 자주 떠올리거나 막 적어 둔 별무리일수록 높은 **흥분성 점수**
 * (시간이 지나면 반감하며 식는다)다. 새 별의 연결은 `semantic + 0.25·흥분성`으로 그 환한
 * 무리 쪽으로 편향되고, 한 무리가 그날 기억을 독식하지 않도록 끌릴 때마다 흥분성을 깎는다
 * (소프트 억제). 아래 설명 카피가 이 기제를 방문자에게 전한다.
 */
export function PresentSelfCard() {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      {/* 진짜 기제 한 줄(spec 22): 토글이 아니라 시간으로 식는 흥분성 점수 + 편향식 + 소프트 억제. */}
      <p className="text-[11px] leading-relaxed text-white/30">
        고르는 건 보여주기 위함이에요. 실은 최근 ~6시간 안에 자주 떠올리거나 막 적어 둔
        별무리일수록 더 환히 깨어 있고(그 빛은 시간이 지나면 스르르 식어요), 새 기억은 의미가
        닿는 별들 가운데 그 환한 무리로 기울어요 — <span className="text-white/45">끌림 = 의미
        유사도 + 0.25 × 흥분성</span>. 한 무리가 오늘의 기억을 다 끌어안지 않도록, 한 번 끌릴
