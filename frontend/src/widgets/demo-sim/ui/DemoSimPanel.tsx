...
const entryChip = 'rounded-full border px-3 py-1.5 text-xs backdrop-blur transition'
const inputCls =
...
function ControlsPanel({
  ...
  const choosePersona = (id: DemoPersona) => {
        ...
        <div className="flex flex-col gap-1.5">
          {demoPersonaList().map((p) => {
            ...
            return (
              ...
              >
                <span className={`text-xs font-medium ${on ? 'text-amber-50' : 'text-white/80'}`}>
                  {p.label}
                </span>
                <span className="text-[11px] leading-snug text-white/45">{p.tagline}</span>
              </button>
            )
          })}
        </div>
  ...
  return (
    ...
    >
      <div
        ...
        className="flex h-[min(85dvh,640px)] w-full max-w-xl flex-col gap-3 rounded-xl border border-white/10 bg-black/80 p-5 backdrop-blur outline-none"
      >
...
export function DemoSimPanel({ initialSimId, onSheetChange }: DemoSimPanelProps) {
  ...
  return (
    <>
      ...
      {theoryOpen && (
        <TheoryModal initialPage={Math.max(0, focusedIdx)} onClose={() => setTheoryOpen(false)} />
      )}
    </>
  )
}
