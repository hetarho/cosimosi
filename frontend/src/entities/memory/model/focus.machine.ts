...
export const focusMachine = setup({
  ...
  actions: {
    ...
    clear: assign({ starId: null, recordId: null }),
  },
}).createMachine({
  id: 'focus',
  context: { starId: null, recordId: null, frameNonce: 0 },
  initial: 'idle',
  // 루트 전이 — 어느 상태에서든 받는다(star↔diary↔idle 자유 전환). 배타는 단일 활성 상태로 보장.
  on: {
