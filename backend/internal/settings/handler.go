...
// UpdateSettings upserts only the present fields and returns the merged result.
// Unknown mood/theme/object or a malformed color → InvalidArgument (no partial write).
func (h *Handler) UpdateSettings(ctx context.Context, req *connect.Request[cosimosiv1.UpdateSettingsRequest]) (*connect.Response[cosimosiv1.UpdateSettingsResponse], error) {
	...
	msg := req.Msg
	p := Patch{Theme: msg.Theme, StarObject: msg.StarObject}
	...
	switch {
	case errors.Is(err, ErrInvalidColor), errors.Is(err, ErrInvalidTheme), errors.Is(err, ErrInvalidObject):
...
func toProtoSettings(s Settings) *cosimosiv1.Settings {
	out := &cosimosiv1.Settings{Theme: s.Theme, StarObject: s.StarObject}
