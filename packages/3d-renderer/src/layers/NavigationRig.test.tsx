...
describe('NavigationRig — pinned', () => {
  ...
  it('opens flat rather than swinging there, since the scene hands it a camera looking straight down', () => {
    ...
    // the allowance, leaving all of the tilt on one side. From the flat there is as much give below
    // as above.
  ...
  it('gives the tilt both of its halves, so the flat has as much give below it as above', () => {
    ...
    // The clamp the controls hold is symmetric about the flat: 15° up, 15° down, 30° in all.
    const orbit = orbits[0] as unknown as { minPolarAngle: number; maxPolarAngle: number }
    expect(Math.PI / 2 - orbit.minPolarAngle).toBeCloseTo(TILT, 10)
    expect(orbit.maxPolarAngle - Math.PI / 2).toBeCloseTo(TILT, 10)

    // And the rig's own envelope agrees with it — a camera dragged below the flat is kept, not
    // pushed back up to level.
    camera.position.set(150 * Math.cos(-TILT), 0, 150 * Math.sin(-TILT))
    run(2)

    expect(offsetFrom([0, 0, 0]).elevation).toBeCloseTo(-TILT, 3)
  })
  ...
  it('eases into the flat when a viewer switches modes, rather than snapping the camera level', () => {
    ...
    // to answer.
    expect(offsetFrom([0, 0, 0]).elevation).toBeGreaterThan(TILT)
    ...
    expect(settled.elevation).toBeLessThanOrEqual(TILT + RIG.arriveEpsilon / settled.radius)
  ...
  it('returns to the pose the viewer chose once the star that took the frame lets go', () => {
    ...
    expect(held.elevation).toBeLessThanOrEqual(TILT + 1e-3)
