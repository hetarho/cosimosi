...
const en_demo_beat_ornament_taster = /** @type {(inputs: Demo_Beat_Ornament_TasterInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Try the place on: the sky behind, the shape a memory takes, the colour a feeling gets.`)
};

const ko_demo_beat_ornament_taster = /** @type {(inputs: Demo_Beat_Ornament_TasterInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`이 공간을 입혀보세요. 뒤의 하늘, 기억이 갖는 모양, 감정이 갖는 색.`)
};

/**
* | output |
* | --- |
* | "Try the place on: the sky behind, the shape a memory takes, the colour a feeling gets." |
*
