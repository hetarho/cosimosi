...
const en_landing_walk_recall_prompt = /** @type {(inputs: Landing_Walk_Recall_PromptInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`Even a faded memory returns the moment it is recalled. Pick up one of the dim ones again?`)
};

const ko_landing_walk_recall_prompt = /** @type {(inputs: Landing_Walk_Recall_PromptInputs) => LocalizedString} */ () => {
	return /** @type {LocalizedString} */ (`흐려진 기억도 다시 떠올리는 순간 돌아와요. 희미해진 별 하나를 골라 회고해 볼까요?`)
};

/**
* | output |
* | --- |
* | "Even a faded memory returns the moment it is recalled. Pick up one of the dim ones again?" |
*
