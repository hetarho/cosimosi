...
- **첫 별 튜토리얼 투어(`DemoGuidedTour`, `widgets/demo-tour`, plan 48·change 34).** 데모 우주(flow=`tutorial`)와 실계정 최초 빈 우주 진입(별 없음·per-user 미완료)이 같은 머신을 공유하며 z-50 overlay를 얹는다 — 현재 target만 남기고 나머지를 어둡게 덮는 **딤(box-shadow spread, 둥근 구멍) + 투명 클릭 차단 패널 4개** + 구멍 둘레의 **빛나는 glow 테두리**(target rect를 `use-tour-target`가 rAF로 추적)와 coach card. 하이라이트된 버튼만 누를 수 있고 coach card만 입력을 받는다. **행동/정보 phase 분리**(change 34): phase `kind`가 행동/정보를 명시해 **행동 phase는 그 행동(버튼·탭·별 클릭·작성 폼 제출)으로만 진행하며 `다음`이 없고**(머신이 `NEXT`를 안 받아 `can('NEXT')=false`), 정보 phase는 이미 열린 표면(작성 폼·조각 검토·망원경 일기 패널·회상 패널)을 버튼이 아니라 **패널 전체로** 하이라이트하고 `다음`으로 넘긴다. **카메라 lock**(change 34): 첫 별 클릭/회상 설명 전까지 마우스·터치·키보드 카메라 조작을 잠근다(`navigation-input.setTourCameraLocked` — 컨트롤러·키보드가 핸들러/프레임에서 read; 별 클릭·폼·HUD는 불간섭). **캔버스 안 생성/fixture 별**은 DOM rect가 없어 universe-canvas가 live 좌표를 화면 rect로 투영해 shared 레지스트리(`shared/lib/tour-target`)에 싣고 투어가 그 rect로 spotlight를 친다(3D 씬 안 `<Html>` 없음 — 헌법8). 시점 전환 항해 실습 구간은 딤·카드를 거의 투명하게 비운다. 단계(surface) 전환 시 페이지가 표면을 정리(또는 작성/회상 표면 유지)하고 UI 숨김도 복구한다. 투어 중에는 모달 백드롭의 바깥-탭-닫기를 끈다.
