...
| 공유 배경 합성(배경 fluid 앞/뒤 + 트윙클 + 별 + Bloom 한 씬) | **widgets** — `widgets/cosmos-scene/ui/CosmosScene.tsx` (plan 43) | 사인인·초대·랜딩이 공유하는 **디커플드 재사용 씬**. prop(별·팔레트·품질)만 주입 — appearance/FSM 미의존(라이브러리화 토대). 별 본체는 `buildStarBody`(uniform 바인딩) 소비, glow는 `BloomPass`(공유). fullscreen fit ortho(`manual=true`), demand+fps 스로틀·quality 다운그레이드. **감정별 별 스튜디오(`/emotion-stars`, change 33)도 이 씬을 재사용** — `StarVisual.stage`(선택) per-star로 13감정 별을 한 캔버스 그리드에 그리고 단계 슬라이더로 단순화 미리보기(13개 별도 컨텍스트 회피). stage 미지정 소비처(랜딩 등)는 단계 0·기본 형태로 무변. |
| 회상 별 포트레이트(전용 미니 Canvas — change 32) | **features** — `features/recall/ui/StarPortrait3D.tsx` | **"한 페이지 한 캔버스" 원칙의 명시적 예외.** 회상 패널이 열렸을 때만 마운트되는 작은 정사각 R3F `<Canvas>`로 클릭한 별 하나를 단일 3D(상단 스포트라이트)로 다시 그린다 — 메인 `UniverseCanvas`와 별개. `frameloop='demand'` + 스로틀 rAF(`star_portrait.fps_cap`)로 비용을 가두고, 패널을 닫으면 언마운트(메인 캔버스 무영향). 본체는 `buildStarBody`(uniform 바인딩)·헤일로는 `buildHalo` 재사용, 단계 버킷은 `entities/star/ui/stage.ts`(`stageBucket`·`formParamsFor`) 공유. 두 번째 렌더러 init 실패가 회상 패널을 날리지 않게 `Sentry.ErrorBoundary`로 가둔다. 조명·카메라·헤일로 수치는 `values.star_portrait.*`(하드코딩 금지) |
