...
- `pnpm dev`는 `concurrently`로 두 프로세스를 동시에 띄운다(`package.json` `scripts.dev`).
  - `web`: `pnpm --filter ./frontend dev` — Vite 개발 서버, `:1214`(`frontend/vite.config.ts` `server.port`).
...
- backend는 `Dockerfile.dev`(golang:1.26-alpine + `air` 핫리로드)로 빌드되고, `./backend`를 볼륨 마운트해 소스 변경을 즉시 반영한다.
...
- Backend
  ...
  - `AI_EMBEDDER` (기본 `mock`) — 임베딩 공급자 선택. `mock`은 keyless 결정론 벡터로 keyless E2E를 가능케 하고, `openai`는 `OPENAI_API_KEY`를 요구한다(`backend/internal/ai/factory.go`).
...
- `CORS_ORIGIN`은 `.env.example`에 없다. compose env로만 주입한다(`docker-compose.yml` backend `CORS_ORIGIN: http://localhost:1214`). config 기본값도 같다(`backend/internal/platform/config/config.go`).
...
- 깨끗한 pgvector DB에 goose `up`을 적용하는 경로는 `scripts/db.mjs`다.
  ...
  - `backend/internal/db/migrations`를 `/migrations`로 마운트하고 `GOOSE_DRIVER=postgres`, `GOOSE_DBSTRING`(compose 내부 호스트 `postgres:5432`)을 넘긴다.
...
- 프론트 빌드: `pnpm --filter ./frontend build` = `tsc -b && vite build`, 0-error.
- 프론트 린트: `pnpm --filter ./frontend lint` = `eslint .`.
- FSD 구조(세그먼트명·public API·슬라이스 경계)는 `pnpm --filter ./frontend lint:fsd` = `steiger ./src`가 별도로 강제한다(`frontend/steiger.config.ts`).

### 임포트 방향 / 순수성 (원칙4)

- ESLint `eslint-plugin-boundaries`가 강제한다(`frontend/eslint.config.js`). 별도 grep이 아니라 `eslint .` 안에서 깨진다.
...
- `@react-three/postprocessing`은 `frontend/package.json` 의존성에도, `frontend/src/**`에도 0건이다. Bloom 등 발광은 TSL 셰이더 경로로만 처리한다(원칙8 "셰이더는 TSL"): Bloom은 `widgets/universe-canvas/ui/BloomPass.tsx`(`three/webgpu` RenderPipeline + `three/addons/tsl/display/BloomNode.js`), emissive TSL 노드는 `entities/star/ui/forms.ts|single.ts`·`entities/synapse/ui`·`widgets/universe-canvas/ui`에 산다. `shared/lib/r3f`는 렌더러 포트(구성)일 뿐 발광 처리 위치가 아니다.
...
- 검사 대상: `backend/internal/db/queries`, `backend/internal/db/migrations`.
- 권장 패턴(주석 줄 제외 — SQL 자기-문서화 주석에 "no UPDATE records" 같은 문구가 패턴에 걸리므로):
  ...
  grep -rniE '\b(delete[[:space:]]+from[[:space:]]+(records|memories|memory_links)|update[[:space:]]+records)\b' \
    backend/internal/db/queries backend/internal/db/migrations | grep -vE ':[0-9]+:[[:space:]]*--'
  ```
  현재 결과 0건.
...
3. 임베딩·연결 — worker가 조각을 임베딩하고 τ=0.75 이상 의미 이웃에 시냅스를 만든다(05). 후보는 `knnK=8` 기준 후보풀 16에서 뽑고, 22의 흥분성 경쟁 재랭크로 최종 `biasedK=5`만 남긴다(`top-8`은 개념적 이웃 수이지 최종 fan-out 개수가 아니다). 임계 SSOT는 `spec/policy/domain/synapse.md`, 쿼리는 `backend/internal/db/queries/embedding.sql`.
...
- `backend/Dockerfile.dev` — golang:1.26-alpine + air
...
- `frontend/eslint.config.js` — `boundaries` pure/renderer/platform 경계 규칙(원칙4)
...
5. `pnpm --filter ./frontend build`와 `lint`가 0-error로 통과한다.
