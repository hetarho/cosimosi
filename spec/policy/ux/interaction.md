...
| **튜토리얼모드(plan 48·change 12·13)** — 모드 선택 `기능 하나하나 알아보기` → flow=`tutorial`. 자유모드 셸 위에 투어 overlay(`widgets/demo-tour`)를 얹어 HUD 버튼을 순서대로(UI 숨김·테마·페르소나·시간·시점·망원경·일기/별 탭·새 별·별 클릭) 짚는다. **행동 안내형**: 단계는 phase로 나뉘어 사용자가 실제 버튼을 눌러 진행한다(UI 숨김 토글·팝오버 열림 관찰; 페르소나/시간은 버튼→팝오버 순으로 하이라이트가 옮겨가 카드가 팝오버를 안 가림). **시점 전환 단계는 항해 실습(change 12)으로 확장** — 멀리서 회전·줌, 가까이서 시선·전진을 디바이스(마우스/터치)별 문구로 안내하고, 사용자가 임계값(`spec/values.yaml` `demo_tour`)을 넘기면 자동 진행한다(신호는 `navigation-input` 누적 카운터·항해 FSM 모드의 rAF 샘플링으로만 관찰 — 매 프레임 React state 금지·헌법4, 항해 조작·물리 불변; 가까이서 전환은 투어가 `TOGGLE_MODE`로 구동, segment 종료 시 멀리서로 정리). 진행은 머신(`tour.machine`)이 소유하고, **행동 phase는 그 행동으로만 진행하며 `다음`이 없다**(머신이 `NEXT`를 안 받아 `can('NEXT')=false` → UI가 버튼을 숨김); 정보 phase만 `다음`으로 넘기고, `건너뛰기`로 투어를 종료한다. 강조는 현재 target만 남기고 나머지를 어둡게 덮어 클릭을 막는 **딤 + 구멍** + 둘레의 **빛나는 glow 테두리**(하이라이트된 버튼만 누를 수 있다). target은 `data-tour-id`로 찾고, 못 찾는 단계(캔버스 안 별·시작/끝)는 딤이 클릭을 막지 않고 중앙 안내 카드만 띄운다. coach card(제목/문구/진행 점/이전/건너뛰기, 정보 phase엔 다음). step은 데모 세션에 저장(새로고침 시 이어감). `건너뛰기`/마지막 `자유롭게 탐험하기`→`free`, 사이드바 `둘러보기 다시 보기`로 재진입 | `DemoGuidedTour` |
