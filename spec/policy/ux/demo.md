...
- **While the tutorial runs, the chrome is gated by one machine.** Only the control the current beat
  ...
  out. Pressability never rides the lift. The tour's guidance floats **at the lower third, at
  readable size** — in the eyeline but under the scene's own middle, so the cluster the words are
  about stays watchable behind them; a step staged inside the writing sheet glues its line to
  the highlighted control itself (just below it, or just above near the bottom edge), and a
  bottom-owning sheet pins it to the top band — guidance never sits on the very surface it
  describes. A beat whose work is a flow rather than one press (the second diary's walk) moves the
  ring with the visitor — 쪼개기, then 띄우기 — instead of staying parked on a button already
  pressed. **The camera is free play's**: drag/zoom navigation stays unmounted while the tour runs,
...
- **Semanticization is earned with pushed time only.** A launch moves the clock to the diary's date
  ...
  or gist body picked on the canvas opens the same entry reader the rail's control does.
- **Free play is real play, in the product's shapes.** The writing control draws a prepared diary
  ...
  recall and semanticization are all experienceable, repeatedly, in any order; opening a launched
  star's entry shows the eroded current words beside the verbatim diary body.
...
- **Recall is per star and repeatable.** Each 회고 re-brightens that star, bumps its recall count,
  reshapes its seed and restores its words — free, with no balance rendered. Each memory's gist
...
- **Decorating is the product's own panel, with the money removed.** The 꾸미기 control opens the
  ...
  Bloom and the camera stay what they always are.
