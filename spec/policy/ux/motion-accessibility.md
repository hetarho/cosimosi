...
| R7 | **우주 셸 오버레이 — 반응형·은은한 딤·배경 탭 복귀·reduced-motion·Esc** — 리스트/탐색 오버레이(`OverlayHost`)와 결과 표면(`Surface`)은 coarse pointer면 바텀시트, fine이면 코너 비의존 떠있는 카드(`FloatingCard`)로 분기한다(분기는 ui 레이어). 포커스/오버레이 상태는 **은은한 딤**(`Backdrop`, 별은 또렷)으로 알리고 **배경 탭으로 복귀**한다: 리스트 펼침은 시트 뒤 차단형 backdrop(바깥 탭=닫기), 별·일기 포커스는 `pointer-events-none` 딤 + 캔버스 `onPointerMissed`(빈 곳 탭=해제, 별 탭·드래그는 통과). 포커스 트랩은 없고 Esc로도 닫는다. reduced 환경에선 슬라이드 스프링·max-height 트랜지션·딤 페이드를 생략해 즉시. 바텀시트 드래그는 핸들에서만 시작(리스트는 네이티브 스크롤)하고, 드래그 직후 합성 click은 핸들 토글을 건너뛴다. | `useCoarsePointer()`=`matchMedia('(pointer: coarse)')`; `Backdrop`(onDismiss=차단형/없으면 pointer-events-none); `<Canvas onPointerMissed>`로 별·일기 해제; `reduce`면 `initial={false}`·duration 0; `useDragControls`+`dragListener={false}`+`didDrag` |
...
- R7: tech/overlay-shell.md · `shared/ui/{OverlayHost,Surface,BottomSheet,FloatingCard,Backdrop}.tsx`·`shared/ui/use-coarse-pointer.ts`, `pages/home/ui/HomePage.tsx`(셸 합성·`useShellStore`·포커스 딤·일기 카드), `widgets/universe-canvas/ui/UniverseCanvas.tsx`(`onPointerMissed` 해제·ViewOffset 일기 시선↑), `features/diary-list/ui/DiaryCard.tsx`
- R8: overlay-shell(tech/overlay-shell.md·모바일 하드닝) · `frontend/index.html`(`viewport-fit=cover`)·`app/styles/index.css`(`#root` dvh)·`pages/home/ui/HomePage.tsx`·`shared/ui/{OverlayHost,BottomSheet,MorningDiffNote}.tsx`·`app/ui/SessionGate.tsx`·`features/switch-appearance/ui/AppearanceSwitcher.tsx`(고정 HUD safe-area offset)
