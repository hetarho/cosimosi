...
| # | 규칙 | 현재 값/조건 |
|---|---|---|
| R1 | **`prefers-reduced-motion: reduce` 존중** — reduced 환경에서 무한 배경/그레인 애니메이션을 정지하고 정적 프레임만 남긴다. 별 밝기·색·시냅스 강도 같은 핵심 의미는 모션 없이도 그대로 전달된다. 랜딩 무대(change 31)는 히어로 엠블럼 전환(스크롤-연동 이동·축소 끔, 두 정적 프레임)·장 연결·전역 물듦·야간 단계가 정적 프레임으로 안착한다. | `useReducedMotion()`(motion/react)로 게이트; reduced 시 `ld-grain--animated`·CosmosScene·랜딩 무대 애니메이션 미적용 |
...
| R7 | **우주 셸 오버레이 — 반응형·은은한 딤·배경 탭 복귀·reduced-motion·Esc** — 탐색·결과 표면(`Surface`)은 coarse pointer면 바텀시트, fine이면 코너 비의존 떠있는 카드(`FloatingCard`)로 분기한다(분기는 ui 레이어). 포커스/오버레이 상태는 **은은한 딤**(`Backdrop`, 별은 또렷)으로 알리고 **배경 탭으로 복귀**한다: 차단형 메뉴/모달 뒤는 backdrop(바깥 탭=닫기), 별·일기 포커스는 `pointer-events-none` 딤 + 캔버스 `onPointerMissed`(빈 곳 탭=해제, 별 탭·드래그는 통과). 포커스 트랩은 없고 Esc로도 닫는다. reduced 환경에선 슬라이드 스프링·max-height 트랜지션·딤 페이드를 생략해 즉시. 바텀시트 드래그는 핸들에서만 시작하고, 드래그 직후 합성 click은 핸들 토글을 건너뛴다. | `useCoarsePointer()`=`matchMedia('(pointer: coarse)')`; `Backdrop`(onDismiss=차단형/없으면 pointer-events-none); `<Canvas onPointerMissed>`로 별·일기 해제; `reduce`면 `initial={false}`·duration 0; `useDragControls`+`dragListener={false}`+`didDrag` |
...
- R1: plan 15 · `widgets/universe-canvas/ui/UniverseGrain.tsx`, `widgets/cosmos-scene/ui/CosmosScene.tsx`, 랜딩 무대(change 31) `pages/landing/ui/StageLayer.tsx`·`pages/landing/ui/section/*`(`useReducedMotion` 게이트)
- R2: plan 15 · `shared/ui/use-coarse-pointer.ts`(coarse pointer 분기는 우주 셸 오버레이가 소비, R7)
...
- R7: tech/overlay-shell.md · `shared/ui/{Surface,BottomSheet,FloatingCard,Backdrop}.tsx`·`shared/ui/use-coarse-pointer.ts`, `pages/home/ui/HomePage.tsx`(셸 합성·포커스 딤·일기 카드), `widgets/universe-canvas/ui/UniverseCanvas.tsx`(`onPointerMissed` 해제·ViewOffset 일기 시선↑), `features/diary-list/ui/DiaryCard.tsx`
- R8: overlay-shell(tech/overlay-shell.md·모바일 하드닝) · `frontend/index.html`(`viewport-fit=cover`)·`app/styles/index.css`(`#root` dvh)·`pages/home/ui/HomePage.tsx`·`shared/ui/{BottomSheet,MorningDiffNote}.tsx`·`app/ui/SessionGate.tsx`·`features/switch-appearance/ui/AppearanceSwitcher.tsx`(고정 HUD safe-area offset)
