...
- **감정색 완료 판정(45)은 per-user·서버 내용으로만.** `EmotionColorGate`는 `GetSettings`(`settingsQueryOptions` 캐시)의 13 mood 감정색 완성 여부로 판정한다 — 최초 로그인 플래그나 localStorage가 아니다. 출처 경계 리셋의 `queryClient.clear()` + `resetServerSettings()`가 settings 캐시·`emotionColors`를 비우므로, 다음 사용자는 자기 `GetSettings`로 다시 판정된다(이전 사용자 색 비노출). 저장은 응답으로 settings 캐시를 `setQueryData` 갱신해 redirect 직후 게이트가 stale empty로 다시 튕기지 않는다.
