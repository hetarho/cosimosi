...
- 출처 경계 리셋: plan 16 · `frontend/src/app/model/{reset-universe-data.ts,auth-store.ts}`, `frontend/src/shared/lib/demo/flag.ts`(`setDemoModeListener`), `frontend/src/features/recall/model/store.ts`(`reset` — 세션 교체·리셋 후 재병합 폐기)
