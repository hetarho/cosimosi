...
| 반지름 = 강함 (38·07·change 18) | 중심 거리 `targetRadius = R_MIN 6 + (R_MAX 80 − R_MIN)·(1 − R^γ)`, `γ = sat_gamma 0.6`(외곽 비-포화 — 오래된 별이 R_MAX에 일찍 안 붙고 달 단위 계속 표류; γ=1이면 옛 선형). **R = Bjork 인출 강도 + 연결성**(`memoryRadiusR` — `R = exp(-Δt/(τ(S)·(1+conn_drift_alpha 0.6·connectedness)))`, `S = (storage_base+recall_count)·(1+emo_consolidation·intensity)`, `τ(S)=tau0_days·(1+tau_storage_gain·ln(1+S))`, `connectedness = degreeNorm + conn_weight_term 0.5·weightedDegreeNorm`). 옛 `strength(W_ACT·act+W_INT·int)` 혼합을 단일 R로 대체(spec 07) — 자주 떠올린·잘 연결된 기억은 같은 Δt에서 더 중앙에 머문다(연결성은 감쇠를 늦추기만, R≤1이라 R_MIN 안으로 못 끌고 무연결 별보다 못 멀어짐). force-sim **반지름 셸 힘**(`SimNode.radius`·`SimParams.radialStrength 0.1`)이 거리를 **독점**하고, 연결·척력은 거리(각도만)를 못 바꾼다(38). R_MAX에 닿은 별은 반지름 정지·각도 드리프트로 회전 |
...
| 흥분성 게인 | `g = 1 + 0.3·arousal`(arousal만; ∈[1,1.3])을 헬퍼(`memory.ExcitabilityGain(arousal)` / `excitabilityGain`)로 정의. Worker는 사용자 전체 별의 Bjork R envelope에서 arousal을 재도출하고 22 할당 바이어스를 `wExc·g`로 라이브 스케일한다 |

### 야간 공고화 (nightly consolidation, 27)

밤마다 사용자별 `consolidate` 잡(`KindConsolidate`)이 한 번 깨어나 우주를 순서 있는 패스로 정돈하고, 아침에 살짝 달라진 풍경(morning diff)을 남긴다 — **무엇도 삭제하지 않는다(헌법2).** 잡은 **야간 티커**가 매일 한 번(`consolidateHourUTC=19` = 04:00 KST) 활성 사용자(별 ≥1)별로 enqueue하고(멱등 — 대기/실행 중이면 미적재), 워커가 `Claim(KindConsolidate)`로 한 건씩(extract·embed 다음 우선순위) 처리한다. 티커는 실제로 배포되는 단일 바이너리(`cmd/api`)와 분리 워커(`cmd/worker`) 양쪽에서 기동된다.

모든 패스는 각 별의 **반지름**(중심 거리)을 축으로 돈다. 좌표는 클라 창발(헌법3)이라 서버는 좌표를 받지 않고, 대신 원자료(`recall_count`·`intensity`·`last_recalled_at`)+시냅스 연결성(중앙값 정규화 degree·Σweight)으로 **change 18 공식**(`R=exp(-Δt/(τ(S)·(1+conn_drift_alpha·connectedness)))`, `targetRadius(R)=R_MIN+(R_MAX−R_MIN)·(1−R^γ)`)을 그대로 포팅해 반지름을 근사한다(`radius.go`). 반지름은 좌표가 아니라 *어느 별을 밤에 건드릴지*만 정한다.

| 패스 | 규칙 / 값 |
|---|---|
| ① 재안정화 (re-stabilize) | **반지름 ≤ `radiusScope=60`인 별과 그 사이 링크만**(`scopeSubgraph`) 서버 force-sim(클라 `shared/lib/force-sim`과 **동일 힘 모델** — 척력 −30·linkDistance 30·centerGravity 0.01·velocityDecay 0.6·alphaMin 0.001)으로 수렴(≤600틱)시킨다. 멀리 표류한(반지름 >60) 별은 sim에 안 넣고 캐시도 안 건드린다(망각 존중). 캐시 좌표가 있으면 시드, 없으면 결정론적 fibonacci 셸 |
| ② 재분배 + spread | 각 스코프 별을 **호스트 성단**(연결 성분, union-find) centroid로 `redistributeLerp=0.6` 끌어들인다(도식 보너스 없음 — change 20 제거). 이어 `spreadClusters`가 성단마다 **결정론 변위**(방향=성단 min 멤버 id FNV 해시 → 단위구 점, 세기 `spreadStrength=6`)를 더해 3D로 분산(한 점 뭉침 방지). RNG·시계 없음 → 같은 입력 그래프면 같은 spread |
| 안정 좌표 캐시 | ①② 결과(스코프 별만)를 `memories.stable_x/y/z`에 upsert. **권위 아님 — 캐시뿐(헌법3).** proto로 클라에 나가지 않는다(`memory.proto` "no coordinate fields"); 서버의 **다음 밤 재진입 시드**로만 재사용한다 |
| ③ 링크 재가중 (reweight) | 시간 기반 연결은 약화, 의미 기반은 강화(시간창은 짧고 의미·도식이 장기 보존). temporal 계열(`intra_entry`·`temporal`) `weight × temporalLinkDecay(0.97)`, `semantic` `LEAST(0.79, weight + semanticLinkGain(0.01))`. `co_recall`(능동 공동회상 헵)·`severed`(끊긴 선) 제외 |
| ④ 요지 (gist) | 별 반지름이 `gistStageRadii([40,55,68,78])` 임계를 넘긴 수(0~4)로 `abstraction_stage`를 `GREATEST(현재, target)`(target>현재만 승급)로 올리고 `version++`. 트리거가 나이/회상 → **반지름**, 연속 `form_seed_delta` → **이산 단계**(change 20). 각 승급을 `evolution_history`에 `trigger='nightly_gist'`(pe 0·dir −1)로 **append**(INSERT 전용). 단계가 형태/AI 변형에 닿는 배선은 plan 53·54 소관(별 도메인 §요지화) |
| ⑤ 가지치기 + 마지막-링크 보호 (prune) | `severed=false`·`weight < weakEdgeThreshold=0.2`·`last_activated_at < now-weakEdgeIdleDays(14)`인 선의 `weight`를 `weakEdgeFloor=0.05`로 **LEAST** + `severed=true`. 단 **별마다 살아있는(미severed) 최강 링크 1개는 보호**(degree ≥ 1 — 완전 고립 금지). 행은 남고 클릭 가능 — **DELETE 0(헌법2)** |
| ⑥ 재-KNN (reconnect) | `created_at < now-reknnMinAgeDays(7)`이고 건강한 링크(severed=false·weight≥0.2)가 없는 고립/끊긴 별의 임베딩으로 의미 KNN(τ=0.75)을 다시 돌려, 닮은 기억과 `semantic` 링크를 새로 만들거나 끊긴 링크를 `severed=false`로 되살린다(가지치기·끊김의 짝 — 재연결 안전망) |
| ⑤b 흥분성 리셋 (24h 회전) | 성단 흥분성 `e(c,t)`(22)는 타임스탬프 파생(τ=6h)이라 **영속 컬럼이 없다** — 아침엔 이미 ≈0. 리셋은 시간 감쇠로 **내재**하며 별도 UPDATE가 없다 |
| 실패 안전 / exactly-once | 한 트랜잭션(cache→reweight→abstract→prune→re-KNN→`CompleteConsolidateJob`)이라 실패 시 통째로 롤백(`failWithBackoff` 재시도). 대부분 멱등(좌표 overwrite·`abstraction_stage` GREATEST+target>현재·`weight` LEAST·재-KNN GREATEST)이나 **재가중은 비-멱등**(곱·합)이라, `CompleteConsolidateJob`이 `status='running'`일 때만 완료하고 0행(다른 워커가 lease 만료로 재claim해 이미 완료)이면 tx를 롤백해 **exactly-once**를 보장한다 |
| morning diff (FE) | 재가중·가지치기로 밝기가 바뀐 선과 재안정화된 좌표가 다음 `GetUniverse` refetch로 자연 반영(요지의 형태 변화는 `abstraction_stage`→형태 배선이 끝나는 plan 53부터). 라이브는 **하루 첫 접속 1회** "밤사이 우주가 한 번 정리됐어요" 노트(localStorage 일자 스탬프·`MorningDiffNote`). 데모는 "하루/한 달 지나기"가 나이 기반 요지·가지치기 근사를 함께 돌려 같은 노트를 띄운다. 안정 좌표는 클라에 안 가므로 좌표는 클라 force-sim 재창발(헌법3) |
...
- 요즘 상태(ambient) 배경 · 감정 weave · 흥분성 게인: 구현 plan 25·07 · `backend/internal/memory/memory.go`(`recall_count`·`ArousalFromSamples`·`ExcitabilityGain(arousal)`)·`db/queries/memory.sql`(`RecallMemoryTouch` +1·`recall_count`·`ListArousalInputs`), `frontend/src/entities/memory/model/{weight.ts,ambient.ts}`(`memoryR`·`rankedEmotions`·`arousalOf`·`deriveAmbient`·`ambientToRgb`·`excitabilityGain`), `frontend/src/widgets/universe-canvas/ui/UniverseNebula.tsx`(감정 weave), `frontend/src/entities/appearance/model/backgrounds.ts`(`emotionSlots`·`pattern`), `backend/internal/job/{repository_pg.go,excitability.go}`(`wExc·ExcitabilityGain(arousal)`)
- 밝기 바닥 · 삭제 없음: 구현 plan 08·12 · `frontend/src/entities/memory/model/activation.ts`(`A_MIN`, `starBrightness`, `synapseBrightness`)
- 야간 공고화 패스(반지름 스코프·재분배·spread·재가중·요지 단계·마지막-링크 보호·재-KNN) · 야간 티커 · 안정 좌표 캐시: 구현 plan 27(change 20) · `backend/internal/job/consolidate.go`(`handleConsolidate`·`scopeSubgraph`·`consolidateLayout`·`redistribute`·`spreadClusters`·`collectReknnLinks`·`StartNightlyConsolidation`)·`radius.go`(`starRadii`·`stageForRadius` — change 18 서버 포팅)·`worker.go`(claim 분기)·`repository.go`/`repository_pg.go`(`GraphStore` 공고화 포트·`Scheduler`)·`db/queries/{memory.sql,link.sql,job.sql}`(`ListStarsForConsolidate`·`CacheStableCoords`·`AbstractStarsByRadius`·`AppendGistHistory`·`ListReknnCandidates`·`ListLinksForConsolidate`·`ReweightLinks`·`PruneWeakLinks`·`ReknnUpsertLinks`·`EnqueueConsolidateJob`·`CompleteConsolidateJob`·`ListActiveUserIDs`)·`db/migrations/00006_nightly_consolidation.sql`(`stable_x/y/z`)·`00013_nightly_rework.sql`(`abstraction_stage`·`severed`)·`cmd/{api,worker}/main.go`(티커 기동) / FE morning diff: `frontend/src/shared/ui/MorningDiffNote.tsx`·`pages/home/ui/HomePage.tsx`·`shared/lib/demo/data.ts`(`demoConsolidate`)·`widgets/demo-sim`(`runConsolidate`·"밤 보내기")
