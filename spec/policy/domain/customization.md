...
| 홈 드래프트·저장 | 홈(우주 메인) 외형 편집은 좌상단 별가루·테마 알약이 여는 집중 모달이며 최상위 `스킨`/`감정 색` 2탭으로 나뉜다(독립 `외형` 헤더 없음, spec 09). **`스킨` 탭**(내부 식별자 `background`/`star`/`self`/`synapse` 그대로, 사용자 용어=스킨)은 **드래프트(라이브 미리보기)** 다 — 잠긴 유료 아이템도 골라 우주에 즉시 미리보고, 자동 저장하지 않는다. 라이브 선택이 마지막 저장(`savedSelection`)과 다르면 저장 바가 한 번에 커밋: 미구매 유료가 있으면 라벨 `저장 · N 별가루`(합계가)·없으면 `저장`, 저장 시 그 아이템들을 먼저 구매(원자)한 뒤 4축 선택을 `UpdateSettings`로 영속한다. 잔액 < N이면 막힌다. "되돌리기"/뒤로(미저장 경고 후 나가기)는 드래프트를 버리고 마지막 저장으로 되돌린다(구매/별가루/저장 규칙 무변). **`감정 색` 탭**은 같은 표면에서 13감정 색을 `EmotionColorEditor`(spec 45)로 편집하고 `UpdateSettings.emotion_colors` 풀세트로 저장한다 — **`감정 색` 저장으로 모달을 떠날 때 커밋되지 않은(미구매) 스킨 드래프트를 되돌려**(`revertSelection`) 미결제 스킨이 활성으로 새지 않게 한다 |
