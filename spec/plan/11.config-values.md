...
> config-values — values.yaml scalar SSOT -> gen:values -> FE/BE constants; build-time only, no hardcoded tuning (status-quo confirmed). | Scope: FS | Status: skeleton — author via /cosimosi:create-plan
> Tag: CAPTURE · Supersedes (.mvp): values.yaml
