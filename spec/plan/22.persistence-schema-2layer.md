# 22. persistence-schema-2layer

> persistence-schema-2layer — Data model for the 2-layer engram: records(immutable) + memories(ensemble) + cells(new) + memory_cells(membership, new) + memory_links + embeddings + jobs, plus repository ports. | Scope: BE | Status: skeleton — author via /cosimosi:create-plan
> Tag: CONCEPT/NEW · Supersedes (.mvp): extends 03
