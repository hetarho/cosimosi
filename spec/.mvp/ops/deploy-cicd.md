...
### 백엔드 프로덕션 이미지 — `apps/api/Dockerfile`
...
- dev의 air 핫리로드(`apps/api/Dockerfile.dev`)와 완전히 분리된다.
- MVP 워커는 같은 바이너리의 고루틴이므로(Architecture §4.6) 한 이미지가 API + 워커를 모두 서비스한다.
- `apps/api/.dockerignore`는 `bin/`, `tmp/`, `*.exe`, `.env`, `.air.toml`, `Dockerfile`, `Dockerfile.dev`를 빌드 컨텍스트에서 제외한다.
...
- `web` 잡: `pnpm/action-setup@v4`(package.json의 packageManager 사용) + node 22 + pnpm 캐시 → `pnpm install --frozen-lockfile` → `pnpm --filter @cosimosi/web lint` → `pnpm --filter @cosimosi/web build`.
- `blog` 잡: 같은 pnpm/node 셋업 → `pnpm install --frozen-lockfile` → `pnpm --filter @cosimosi/blog build`.
- `api` 잡(`working-directory: apps/api`): `setup-go@v5`(`go-version-file: apps/api/go.mod`) → `go vet ./...` → `go build ./...`. Linux 러너라 §8 Windows .exe 제약과 무관.

### 백엔드 배포 — `.github/workflows/deploy-backend.yml`

- 트리거: `on: push: branches: [develop, main]` + `paths: ['apps/api/**', 'docker-compose.prod.yml', '.github/workflows/deploy-backend.yml']` + `workflow_dispatch`(수동 재배포). edge Caddyfile은 트리거 경로가 아니다(별도 수동 관리).
...
- **`build` 잡** (`permissions: packages: write`):
  - `docker/build-push-action@v6`로 `./apps/api/Dockerfile`을 항상 빌드해 Dockerfile을 CI에서 검증한다.
...
- **`rollout` 잡** (`needs: build`, `if: vars.DEPLOY_ENABLED == 'true'`, `environment:`로 per-env 시크릿 + prod 수동 승인 옵션):
  1. `appleboy/scp-action@v0.1.7`로 마이그레이션 동기화: `source: apps/api/internal/db/migrations/*` → `target: /srv/cosimosi-${ENV_NAME}/migrations`, `strip_components: 5`(경로 평탄화 — `apps/api`가 `backend`보다 한 단계 깊어 한 컴포넌트를 더 벗긴다). goose 실행 이전에 둔다(goose가 마운트하므로 파일이 먼저 있어야 한다).
...
- `wrangler.jsonc`(리포 루트): `name: cosimosi`, `assets.directory: ./apps/web/dist`, `assets.not_found_handling: single-page-application`(딥링크/새로고침을 200 index.html로). 별도 Worker 스크립트(`main`)는 없다 — 순수 정적 Vite SPA.
- 대시보드 Build command `pnpm --filter @cosimosi/web build` → `apps/web/dist`, deploy `npx wrangler deploy`(main 프로덕션 승격) / `npx wrangler versions upload`(그 외 브랜치 프리뷰).
...
- **BE** (`apps/api/cmd/api/main.go`): `cfg.SentryDSN`이 있으면 `sentry.Init(ClientOptions{Dsn, Environment, Release: version})`. 성공 시 `defer sentry.Flush(2s)` + `defer sentry.Recover()`. DSN 없으면 전체를 건너뛴다(로컬/테스트 no-op).
...
- **FE** (`apps/web/src/main.tsx`): `VITE_SENTRY_DSN`이 있으면 `@sentry/react` `Sentry.init`. 현재 18이 강화한 상태다 — `environment: VITE_SENTRY_ENVIRONMENT ?? MODE`, `release: VITE_APP_VERSION`(없으면 undefined), `integrations: [browserTracingIntegration()]`, `tracesSampleRate: 0.1`. DSN 없으면 init 스킵. PostHog `initAnalytics({key, host})`도 같은 엔트리에서 호출하지만 그 소관은 18이다.
  - `tracePropagationTargets`는 기본(동일 출처)이다 — 교차 출처 api에 sentry-trace 헤더를 붙이면 백엔드 CORS allow-headers에 없어 프리플라이트가 깨진다.
  - `VITE_APP_VERSION`은 `apps/web/vite.config.ts`가 `WORKERS_CI_COMMIT_SHA`에서 주입한다(18 소관).
...
- `wrangler.jsonc`
  - `name: cosimosi`, `assets.directory: ./apps/web/dist`, `not_found_handling: single-page-application`
...
- `apps/api/internal/platform/rpcserver`
  - `PanicCapture` 훅 타입
