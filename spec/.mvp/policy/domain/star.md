...
| 규칙 | 값 / 조건 |
|---|---|
| 단일 축 룩 + 단계 지오메트리 (spec 53, change 29) | 별 형태는 **단일 축 "룩" 3종**: `polyhedron`(다면체)·`liquid`(액체→구름)·`spiky`(고슴도치). 각 룩이 모양+질감+추상화 단계 변형을 한 덩어리로 정의한다(`buildStarBody(look, stage)`, `STAR_LOOK_BUILDERS`; shared toolkit plan 50 geometry 생성기 `polyhedronForStage`·`spikyGeometry` 조립). 기본 무료 `polyhedron`. **선택 wire(`star_object`)는 전역 기본 룩 id 하나** — 합성 `"form+surface"`·프리셋 디컴포지션 폐기(미지/레거시는 디폴트 룩 폴백, 호환 테이블 없음). 렌더는 **(룩×단계) 버킷별 `InstancedMesh` 집합**(메시 개수 = 사용 룩 수 × 단계, 별 수 무관 상수, 헌법8 개정) + TSL 노드 머티리얼. **커스터마이즈 '별' 축 = 단일 룩 판매**(무료 1+유료 N, 아이템 `star:look:<id>`). **색은 mood 불변**(형태·질감만). 나·시냅스는 form×surface 2축 유지(spec 52). [customization](customization.md) |
| 감정별 형태 오버라이드 (change 30) | 별 룩은 **전역 기본 + 감정(13 mood)별 오버라이드**다. 렌더 룩 = **그 별 mood의 오버라이드 룩 ?? 전역 기본(`star_object`)** — StarField가 (룩×단계) 버킷을 *사용 중인 룩 집합*(전역 ∪ 오버라이드 값)으로 일반화해 라우팅한다(오버라이드 0개면 전역 룩 하나라 단일-룩 동치, 헌법8 유지). 서버 권위 맵 `Settings.emotion_forms`(proto 필드 6 append-only)·DB `user_emotion_forms`(00015) — 색 오버라이드(`emotion_colors`)의 평행물, "바꾼 mood만" 부분 업서트. 미지 mood·미지 룩은 전역 기본으로 안전 폴백(`parseStarLook`, 크래시 없음). 소유권은 전역 축과 같은 `star:look:<id>` 규칙(미소유 룩 배정은 BE가 `ErrNotOwned`로 거부; 한 번 소유하면 어느 감정에든 자유 배정). **색·크기·밝기·시드 규칙 불변**(형태 선택 차원만). [customization](customization.md) |
