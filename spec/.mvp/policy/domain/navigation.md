...
- fly-to 보간(`k=1−exp(−dt·3)`, 오프셋 12, 임계 0.6)·잠든 별 도달·항행 머신 `FLY_TO_STAR`·포커스 머신 `SELECT_STAR`: 구현 plan 12 · `frontend/src/widgets/universe-canvas/ui/controllers/flights.tsx`(`FlyToController`/`FocusController`), `frontend/src/features/star-explorer`.
- 우주 셸 영속·탐색=페이지 로컬 표면 상태(캔버스 비재초기화)·`?panel=` 레거시 1회 리다이렉트·`?record=` 일기 페이지 handoff(1회 소비+clear): 구현 tech/overlay-shell.md · `frontend/src/pages/home/ui/HomePage.tsx`(셸 합성·`?record=`/`?fly=` 1회 소비), `frontend/src/pages/diary/`(`/diary` 페이지·"우주에서 보기"), `frontend/src/shared/ui/{Surface,Backdrop,SideDrawer}.tsx`, `frontend/src/app/router.tsx`(index 라우트 검증·`?panel=` 레거시 리다이렉트·`/diary` 보호 라우트).
