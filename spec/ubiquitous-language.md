...
| `Ornament`                                     | 장식 아이템     | 카탈로그 — `OrnamentKind` + 획득 경로. 카탈로그는 **테이블이 아니라 Go 코드**이며, 가격은 행이 아니라 종류가 정한다. `GENERAL` 별가루로만 구매하며 영구 소유(환불·기간제 없음). 시적: _우주 꾸미기 아이템_.                                                                                                                                                                     |
| `OrnamentKind`                                 | 장식 종류       | `BACKGROUND`(배경) / `STAR_SHADER`(별 몸체) / `GIST_SHADER`(요지 몸체) / `MOTE`(배경 입자 하나) / `MOTE_FIELD`(그 입자들이 뿌려지는 공간) 다섯으로 닫힌 집합. 이름은 **꾸며지는 자리**이지 그 자리를 채우는 렌더링 명사가 아니다(§4의 `StarShape`·`GistShape`·`BackdropMote`·`BackdropField`는 FE에만 산다). 감정 색은 꾸미기가 아니다 — `MoodColor`가 따로 소유하며 전부 무료. |
...
| `AchievementProgress` (`achievement_progress`) | 업적 진행       | 파생 불가능한 세 사실 — 달성 시각 · **수령 요청**(`claimed_at`) · **보상 지급 완료**(`paid_at`). 수령 요청은 지급보다 먼저 커밋되므로 둘은 별개 사실이다. 진행도 자체는 저장하지 않고 읽을 때 계산한다. 수령은 명시적 행위.                                                                                                                                                     |
...
| `SequenceStep`                                 | 시퀀스 스텝     | **FE 전용 · DB 없음.** 가이드 한 걸음 — 안내 문구 · 가리킬 컨트롤 · 진행 조건뿐이다. 동작 필드가 없다: 엔진은 가리키고 기다리기만 한다.                                                                                                                                                                                                                                         |
| `SequenceScript`                               | 시퀀스 스크립트 | **FE 전용 · DB 없음.** `SequenceStep`의 순서 있는 목록. 스크립트는 **걸음**을 지정하고 `DemoScenario`는 **데이터**를 지정한다 — 데모와 온보딩은 이 둘만 다르다.                                                                                                                                                                                                                 |
| `SequenceRun`                                  | 시퀀스 실행     | **FE 전용 · DB 없음.** 스크립트 한 번의 진행. 결과는 `completed` / `skipped` / `abandoned`. 아무것도 영속되지 않으므로 재생은 초기화가 아니라 새 실행이다.                                                                                                                                                                                                                      |
...
| `ClaimAchievement` | 업적 보상 수령  | 달성한 업적의 `claimed_at`을 먼저 기록·커밋한 뒤 보상을 지급하고 `paid_at`을 찍는다. 자동 지급하지 않는다.                         |
| `SettleClaim`      | 보상 지급 확정  | 지급된 보상에 `paid_at`을 찍는다. 수령했으나 미지급인 행은 워커가 이 동작으로 배수(drain)한다 — 두 지급 경로 모두 `claim_id` 멱등. |
...
>
> **꾸미기(`Ornament`)가 바꾸는 것은 이 레이어의 렌더 파라미터뿐이다** — 배경·별 셰이더는 위치·밝기·크기·감정 색의
> 의미·형태 seed를 덮어쓰지 않으며, 렌더 어휘가 도메인으로 역류하지도 않는다. 감정 색 변경(`MoodColor`)은 꾸미기가
> 아니며, mood→color 매핑의 단일 진입점만 갈아끼운다.
...
| `StarShape`     | 별 모양   | `star` 몸체의 생김새 하나(`STAR_SHAPES` 레지스트리의 한 항목). 렌더 전용 — 서버는 `star_shader.<key>` 불투명 id만 알고, 형태·기복·노이즈·표면 움직임만 정하며 tint·밝기·seed·크기에는 쓰기 권한이 없다.                                                                                                                |
| `gist-star`     | 요지 별   | 요지(신피질) 몸체 = 떠오른 `SemanticMemory` 하나. seed 없음 — 우주 안의 모든 요지 별은 같은 형태이고, 서로를 가르는 건 감정 tint와 올라간 깊이뿐이다. 크기는 `EffectiveStrength`, 확산도는 요지 단계.                                                                                                                  |
| `GistShape`     | 요지 모양 | `gist-star` 몸체의 생김새 하나(`GIST_SHAPES` 레지스트리의 한 항목). 렌더 전용 — 서버는 `gist_shader.<key>` 불투명 id만 안다. 빛만 바꾼다 — 정점을 움직이지 않고, seed를 읽지 않으며, tint·확산도·크기에는 쓰기 권한이 없다. 우주 전체에 하나뿐인 선택이다([V5]).                                                       |
| `BackdropMote`  | 먼지 하나 | 우주 뒤 장식용 입자 **하나**의 정체(`BACKDROP_MOTES` 레지스트리의 한 항목) — 형태(form) + 색(tone). 크기는 행에도 선택에도 없다: 모든 장이 `BACKDROP_MOTE_SIZE_MIX`(1·2·3·4)를 정해진 비율로 한꺼번에 두른다. 어디에 몇 개가 뿌려지는지는 모른다. 도메인 데이터 없음·렌더 전용 — 서버는 `mote.<key>` 불투명 id만 안다. |
| `BackdropField` | 먼지 장   | 그 입자들이 뿌려지는 **공간**(`BACKDROP_FIELDS` 레지스트리의 한 항목) — 배치(scatter)·밀도(density)·반짝임(life + 속도·깊이)·회전. 입자 하나의 설정이 아니라 장 전체의 성격이라, "빼곡함"과 "아예 없음"도 한 항목이다. 렌더 전용 — 서버는 `mote_field.<key>` 불투명 id만 안다.                                         |
