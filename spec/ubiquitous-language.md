...
| `GistShape`     | 요지 모양 | `gist-star` 몸체의 생김새 하나(`GIST_SHAPES` 레지스트리의 한 항목). 렌더 전용. 빛만 바꾼다 — 정점을 움직이지 않고, seed를 읽지 않으며, tint·확산도·크기에는 쓰기 권한이 없다.                                                                                  |
| `BackdropMote`  | 먼지 하나 | 우주 뒤 장식용 입자 **하나**의 정체(`BACKDROP_MOTES` 레지스트리의 한 항목) — 형태(form)·크기(size)·색(tone) 셋이 한 몸이다. 어디에 몇 개가 뿌려지는지는 모른다. 도메인 데이터 없음·렌더 전용.                                                                  |
| `BackdropField` | 먼지 장   | 그 입자들이 뿌려지는 **공간**(`BACKDROP_FIELDS` 레지스트리의 한 항목) — 배치(scatter)·밀도(density)·반짝임(life + 속도·깊이)·회전. 입자 하나의 설정이 아니라 장 전체의 성격이라, "빼곡함"과 "아예 없음"도 한 항목이다. 렌더 전용.                              |
| `BackdropTheme` | 배경 테마 | 이름 붙은 `BackdropMote` × `BackdropField` **짝** 하나(`BACKDROP_THEMES` 레지스트리의 한 항목). 자기 파라미터는 없다 — 어느 카탈로그에도 없는 게 필요하면 축에 추가한다. 제품 화면이 입을 수 있는 건 이 짝뿐이고, 삼각형 상한도 짝에 걸린다.                   |
