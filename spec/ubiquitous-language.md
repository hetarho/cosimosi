...
- **과학 용어 규칙은 `memory` 컨텍스트에만 적용된다.** `identity`(User·Auth·Invite) · `store`(Ornament) ·
...
- **신규 도메인 용어는 코드보다 이 문서에 먼저.** 단 지원 컨텍스트(`identity` · `store` · `achievement` · `demo`)의
...
| `EpisodicMemory` (`episodic_memories`)    | 일화기억             | 한 경험의 기억 = 뉴런들의 _희소 앙상블_. **애그리거트 루트**(뉴런을 id로 참조). `name`(LLM 추천, 사용자 수정 가능)·`Emotion`·`CurrentText`·`SemanticStages`·`DecayStages` 보유. 시적: _기억의 별(○○별)_. |
...
### 1.1 지원 컨텍스트 명사 (`identity` · `store` · `achievement` · `demo`)
...
| `Withdraw`         | 탈퇴            | 계정 소프트 딜리트 30일(이후 하드 삭제). 확인 화면에서 Export를 안내한다.                                                |
