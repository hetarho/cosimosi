...
| `BackdropTheme` | 배경 테마 | 우주 뒤 장식용 입자 장(場) **전체**의 성격 하나(`BACKDROP_THEMES` 레지스트리의 한 항목). 네 축(scatter·mote·life·tone) + 밀도·크기·회전의 배열. 입자 하나의 설정이 아니라 장 전체의 성격이라, "빼곡함"과 "아예 없음"도 한 항목이다. 도메인 데이터 없음·렌더 전용. |
