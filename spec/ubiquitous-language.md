...
| `BackdropMote`  | 먼지 하나 | 우주 뒤 장식용 입자 **하나**의 정체(`BACKDROP_MOTES` 레지스트리의 한 항목) — 형태(form) + 색(tone). 크기는 행에 박지 않고 `BACKDROP_MOTE_SIZES`(1·2·3·4)에서 고른다 — 모든 형태가 모든 크기로 볼 가치가 있으니까. 어디에 몇 개가 뿌려지는지는 모른다. 도메인 데이터 없음·렌더 전용. |
