...
(Dockerfile, `docker-compose.prod.yml`, `deploy/edge/`, `.github/workflows/`)은 리포 안에 있고,
...
VPS 한 대가 두 스택을 모두 호스팅한다(디렉터리·서브도메인 분리, TLS는 공유 edge Caddy 하나가 두
도메인 모두 발급 — 아래 4). Lightsail을
...
3. **Docker 설치**: SSH 접속 후 Docker Engine + compose 플러그인 설치(get.docker.com 스크립트 →
   ...
   만든다(1GB 플랜 안전판). `docker compose version`으로 확인. 공유 프록시용 외부 네트워크 생성:
   `docker network create edge`.
4. **공유 edge Caddy (80/443의 유일한 소유자)**: `/srv/edge/`에 리포의
   `deploy/edge/{docker-compose.yml,Caddyfile}`을 복사하고, 비추적 `.env`에
   `API_DOMAIN_PROD=api.<도메인>`·`API_DOMAIN_STAGING=api.staging.<도메인>`을 적은 뒤
   `docker compose up -d`. **스택마다 Caddy를 띄우지 않는다** — 두 caddy가 80/443을 두고 충돌한다.
   api 스택들은 `edge` 네트워크 별칭(`cosimosi-api-prod`/`cosimosi-api-staging`)으로 연결되고,
   한쪽 api가 내려가도 다른 도메인은 계속 서빙된다. 이 스택은 배포 워크플로 밖(수동 관리)이다.
5. 각 환경 `e ∈ {staging, prod}`마다 `/srv/cosimosi-$e/` 디렉터리에 아래를 둔다:
   - `docker-compose.prod.yml` (이 리포에서 복사 — 파일이 바뀌면 재복사한다. 배포 워크플로는
     migrations만 동기화한다.)
   - `.env` — `.env.production.example`을 채운 것(비추적, `chmod 600`).
     `API_UPSTREAM`(`cosimosi-api-staging` / `cosimosi-api-prod`), `DATABASE_URL`(6543),
     `CORS_ORIGIN`(해당 환경 프론트 도메인), `SUPABASE_PROJECT_URL`(+필요시 `SUPABASE_JWT_SECRET`),
     `AI_EMBEDDER`/`OPENAI_API_KEY`, `SENTRY_DSN`/`SENTRY_ENVIRONMENT` 설정. `IMAGE_TAG`는 배포
     스텝이 덮어쓴다.
   - `migrations/` — goose 실행 전에 배포 워크플로(scp)가 생성/동기화한다.
6. 배포용 SSH **공개키**를 인스턴스 기본 사용자(`ubuntu`)의 `~/.ssh/authorized_keys`에 등록.
7. `docker login ghcr.io`를 패키지 **read** 권한 토큰으로 1회 실행(`compose pull`이 되도록 —
   push는 워크플로가 `GITHUB_TOKEN`으로 처리). ⚠️ **sudo 없이**(`ubuntu` 계정으로) — 배포가
   `ubuntu`로 `compose pull`을 실행하므로 자격증명이 그 계정의 `~/.docker`에 있어야 한다.
8. **DNS**: `api.<도메인>`·`api.staging.<도메인>`의 `A` 레코드를 Lightsail **고정 IP**로 향하게
